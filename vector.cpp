...
int partitions1(vector<int>, int,int);
void quickSort(vector<int>,int,int);
void heapSort(vector<int>,int);
void swap1(int*, int*);
static int merge_swap=0,quick_swap=0;
...
//-----------time of an quick sort------------------------
                ...
                cout<<"\ntime(best case) : "<<endtime-starttime<<" second"<<endl;
                cout<<"\ntime(avg case) : "<<((tmp_time+endtime-starttime)/2)<<" second"<<endl<<endl;
                cout<<"swap(worst case) : "<<quick_swap<<endl;
                cout<<"Time Difference(sorted and unsorted array) "<<tmp_time-endtime+starttime<<" second"<<endl;
...
int partitions1(vector<int> arr1,int n,int m)  
{  
    int pivot = arr1[m]; // pivot  
    int i = (n - 1); // Index of smaller element  
...
quick_swap++;
    ...
    swap1(&arr1[i + 1], &arr1[m]);  
quick_swap++;
    return (i + 1);  
}  
void quickSort(vector<int> arr1,int n,int m)  
{  
    if (n < m)  
    {  
        /* pi is partitioning index, arr[p] is now  
        at right place */
        int pi = partitions1(arr1,n,m); 
  
        // Separately sort elements before  
        // partition and after partition  
        quickSort(arr1, n, pi - 1);  
        quickSort(arr1, pi + 1, m);  
    }  
} 
